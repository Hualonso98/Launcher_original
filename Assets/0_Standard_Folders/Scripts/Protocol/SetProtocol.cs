...
public class SetProtocol : MonoBehaviour
{
    ...
    [Space]
    public Button addExercise_button;
    public Button addStep_button;
  ...
  //  int patientId = -1; //CAMBIO DE ID A ALFANUMÉRICO
    ...
    Coroutine tab_input_cor;
    Coroutine enter_input_cor;
    ...
    IEnumerator CorrutinaStart()
    {
        ...
        {
            addExercise_button.interactable = false;
            addExercise_button.GetComponentInChildren<Image>().color = Color.grey;
        }

        if (numReps[0].text == "")
        {
            addStep_button.interactable = false;
        }
        if (numReps[1].text == "")
        {
            addStep_button.interactable = false;
    ...
    public void VerifyProtocolAlreadyExists()
    {
        ...
        if (File.Exists(path))
        {
            ...
            if (aux.Protocol_finished == false)
            {
            ...
            {
                Debug.Log("No existe protocolo para el paciente, o se ha terminado");
    ...
    public void EditDropList()
    {
        ...
        for (int i = 0; i < protocols.Protocols_list.Count; i++)
        {
            ...
            Debug.Log("EditDropList: " + protocols.Protocols_list[i].Name + " " + protocols.Protocols_list[i].Exercises[0]);
        }
    ...
    public void ChangeProtocolDrop(int value)
    {
        if (value != 0)
        {
            ...
            {
                Debug.Log(protocols.Protocols_list[value - 1].Name);
    ...
    public void OpenAddProtocolPanel()
    {
        ...
        if (panel_add_protocol.activeSelf)
        {
            ...
            tab_input_cor = StartCoroutine(TabInputs());
            enter_input_cor = StartCoroutine(EnterInput());
        }
        else
        {
            StopCoroutine(tab_input_cor);
            StopCoroutine(enter_input_cor);
        }
    }
    ...
    IEnumerator TabInputs()
    {
        ...
        if (EventSystem.current.currentSelectedGameObject == exerciseDrop.gameObject && addExercise_button.interactable)
        {
            AddExercise();
        }
    ...
    IEnumerator EnterInput()
    {
        ...
        if (numReps[1].isFocused && addStep_button.interactable)
        {
            AddStepFunc();
        }

        enter_input_cor = null;
        enter_input_cor = StartCoroutine(EnterInput());
    }
    ...
    public void AddExercise()
    {
        if (!numReps[0].IsInteractable())
        {
            ...
            if ((numReps[0].text != "" && int.Parse(numReps[0].text) != 0) || (numReps[1].text != "" && int.Parse(numReps[1].text) != 0)) { addStep_button.interactable = true; }
            addExercise_button.transform.localScale = new Vector3(-1, 1, 1);
    ...
    public void ChangeAppDrop(int value)
    {
        if (value != 0)
        {
            addExercise_button.interactable = true;
            addExercise_button.GetComponentInChildren<Image>().color = Color.white;
        }
        else
        {
            addExercise_button.interactable = false;
    ...
    public void ChangeRepsInput(string value)
    {
        ...
        {
            addStep_button.interactable = true;
        }
        else
        {
            addStep_button.interactable = false;
    ...
    public void AddProtocol()
    {
        if (exercises.Count != 0)
        {
            if (FindProtocolByName(protocol_name.text) == false)
            {
                ...
                new_protocol.Exercises = new List<string>(exercises); //Hecho así para que no se sobreescriba luego
                new_protocol.Left_reps = new List<string>(left_reps);
                new_protocol.Left_reps_done = Enumerable.Repeat("0", left_reps.Count).ToList(); /*new List<string>(left_reps.Count);*/
                new_protocol.Right_reps = new List<string>(right_reps);
                ...
                Debug.Log("Protocol: " + new_protocol.Name + " " + new_protocol.Exercises[0]);

                protocols.Protocols_list.Add(new_protocol);

                Debug.Log("Pre_sort: " + protocols.Protocols_list[0].Name + " " + protocols.Protocols_list[0].Exercises[0]);
                try { Debug.Log("Pre_sort: " + protocols.Protocols_list[1].Name + " " + protocols.Protocols_list[1].Exercises[0]); } catch { }
                protocols.Protocols_list.Sort(SortProtocolByName); //La ordeno según el nombre

                Debug.Log("Post_sort: " + protocols.Protocols_list[0].Name + " " + protocols.Protocols_list[0].Exercises[0]);
                try
                {

                    Debug.Log("Post_sort: " + protocols.Protocols_list[1].Name + " " + protocols.Protocols_list[1].Exercises[0]);
                }
                catch { }
...
{
    [SerializeField] bool protocol_finished = false;
    ...
    public bool Protocol_finished { get => protocol_finished; set => protocol_finished = value; }
}
